...
**2. New capabilities** (each skill *body* is on-demand — zero cost until invoked; their descriptions are the registry cost already counted in the −39 net above). Each is authored to the `writing-skills` doctrine and **validated**: all are structurally checked (lint-clean, balanced fences, resolvable links, trigger-only descriptions), and the seven new skills with a first-response trigger (`writing-tests`, `e2e-testing`, `adversarial-audit`, `orchestration-routing`, and the three `*-standards` skills) pass live triggering tests (`plugin/tests/skill-triggering`). Only `concise-output` — an output-discipline meta-skill invoked while producing output — is exercised via cross-references instead:
